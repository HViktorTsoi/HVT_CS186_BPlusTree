...
public class BPlusTree {
    ...
    public void put(DataBox key, RecordId rid) throws BPlusTreeException {
        typecheck(key);
        Optional<Pair<DataBox, Integer>> rst = root.put(key, rid);
        //Overflow happens, root changes
        if (!rst.equals(Optional.empty())) {
            List<DataBox> keys = new ArrayList<>();
            List<Integer> children = new ArrayList<>();
            keys.add(rst.get().getFirst());
            //新的root的第一个children就是老root
            children.add(root.getPage().getPageNum());
            children.add(rst.get().getSecond());
            //updata root（innerNode）
